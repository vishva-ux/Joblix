...
async function runTests() {
  ...
  const claimAttempts = await Promise.all(
    workersList.map(async (workerId) => {
      try {
  ...
  // Cleanup
  await prisma.deadLetterQueue.deleteMany({ where: { queueId: testQueue.id } });
  await prisma.jobExecution.deleteMany({ where: { jobId: testJob.id } });
  await prisma.job.deleteMany({ where: { queueId: testQueue.id } });
  await prisma.worker.deleteMany({
    where: { id: { in: ['test-worker-node', ...workersList] } }
  });
  await prisma.retryPolicy.deleteMany({ where: { queueId: testQueue.id } });
  await prisma.queue.delete({ where: { id: testQueue.id } });
