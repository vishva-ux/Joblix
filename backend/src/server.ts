  ...
  try {
    ...
    const project = await prisma.project.create({
      data: {
        ...
        apiKey: `joblix_${uuidv4().replace(/-/g, '')}`
      }
    });
...
app.get('/api/projects', authMiddleware, async (req: AuthRequest, res: express.Response) => {
  try {
...
app.post('/api/projects', authMiddleware, async (req: AuthRequest, res: express.Response) => {
...
app.get('/api/queues', authMiddleware, async (req: AuthRequest, res: express.Response) => {
  ...
  try {
    ...
    const enrichedQueues = await Promise.all(
      queues.map(async (queue) => {
        ...
        const statusCounts = counts.reduce((acc: any, curr) => {
...
app.post('/api/queues', authMiddleware, async (req: AuthRequest, res: express.Response) => {
...
app.patch('/api/queues/:id', authMiddleware, async (req: AuthRequest, res: express.Response) => {
  const { id } = req.params;
...
app.delete('/api/queues/:id', authMiddleware, async (req: AuthRequest, res: express.Response) => {
  const { id } = req.params;
  try {
...
async function getProjectFromRequest(req: AuthRequest): Promise<string | null> {
  if (req.projectId) return req.projectId; // set by apiKeyMiddleware
  const { projectId } = req.query;
...
// Queue / Create jobs
app.post('/api/jobs', async (req: AuthRequest, res: express.Response, next) => {
...
app.post('/api/jobs/batch', authMiddleware, async (req: AuthRequest, res: express.Response) => {
...
app.get('/api/jobs', authMiddleware, async (req: AuthRequest, res: express.Response) => {
...
app.get('/api/jobs/:id', authMiddleware, async (req: AuthRequest, res: express.Response) => {
  const { id } = req.params;
...
app.post('/api/jobs/:id/retry', authMiddleware, async (req: AuthRequest, res: express.Response) => {
  const { id } = req.params;
...
app.post('/api/jobs/:id/cancel', authMiddleware, async (req: AuthRequest, res: express.Response) => {
  const { id } = req.params;
...
app.get('/api/workers', authMiddleware, async (req: AuthRequest, res: express.Response) => {
  try {
...
app.get('/api/metrics', authMiddleware, async (req: AuthRequest, res: express.Response) => {
  const { projectId } = req.query;
  if (!projectId || typeof projectId !== 'string') {
    return res.status(400).json({ error: 'Missing projectId' });
  }
  ...
  try {
    ...
    const counts = jobStats.reduce((acc: any, curr) => {
      acc[curr.status] = curr._count;
      return acc;
    }, { QUEUED: 0, SCHEDULED: 0, CLAIMED: 0, RUNNING: 0, COMPLETED: 0, FAILED: 0 });

    const totalJobs = Object.values(counts).reduce((a: any, b: any) => a + b, 0);
...
app.get('/api/dlq', authMiddleware, async (req: AuthRequest, res: express.Response) => {
  const { projectId } = req.query;
  if (!projectId || typeof projectId !== 'string') {
    return res.status(400).json({ error: 'Missing projectId' });
  }
...
app.get('/api/jobs/:id/ai-summary', authMiddleware, async (req: AuthRequest, res: express.Response) => {
  const { id } = req.params;
