...
const WORKER_ID = process.env.WORKER_ID || `worker-${os_1.default.hostname()}-${process.pid}`;
...
async function sendHeartbeat() {
    ...
    try {
        ...
        await db_1.default.worker.upsert({
            where: { id: WORKER_ID },
            update: {
            ...
            create: {
                id: WORKER_ID,
                name: `Worker Node [${os_1.default.hostname()}]`,
                status: 'ACTIVE',
                concurrency: CONCURRENCY,
                lastHeartbeat: new Date()
            }
...
async function claimJob() {
    if (isShuttingDown || activeJobs >= CONCURRENCY)
        return null;
    try {
    ...
    const runPromise = (async () => {
        ...
        try {
            ...
            if (job.cronExpression) {
                try {
                    const interval = cron_parser_1.default.parseExpression(job.cronExpression);
                    const nextRun = interval.next().toDate();
                    // Create new scheduled Job
                    const nextJob = await db_1.default.job.create({
                        data: {
                            queueId: job.queueId,
                            status: 'SCHEDULED',
                            payload: job.payload,
...
async function gracefulShutdown(signal) {
    ...
    try {
        ...
        await db_1.default.worker.update({
            where: { id: WORKER_ID },
